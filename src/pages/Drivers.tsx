...
export type DriverStatus = "active" | "inactive" | "suspended" | "pending" | "blocked";
...
const DATA: Driver[] = [
    ...
    },
    {
      ...
      "profilePicUrl": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRvPx5Ngn3BqU_b1o4MO5-90QnJXVEdVLYmaA&s",
      "dob": "1992-05-21",
      "gender": "male",
      "address": {
  ...
  const filterFields = [
    ...
    { key: "license_expiry_date", label: "License Expiry Date", type: "date" as const },
  ];

  const filteredData = DATA.filter((user) => {
    if (filters.status.length > 0 && !filters.status.includes(user.status as DriverStatus)) {
      return false;
    }
    if (filters.joined_at && !isSameDay(new Date(user.createdAt), filters.joined_at)) {
      return false;
    }
    if (
      filters.license_expiry_date &&
      !isSameDay(new Date(user.documents?.[0]?.expiryDate), filters.license_expiry_date)
    ) {
      return false;
    }
    return true;
  ...
  return (
    <div className="w-full h-full flex flex-col p-[10px] gap-[6px]">
      <Filter<Filters> fields={filterFields} initialValues={filters} onChange={setFilters} />
